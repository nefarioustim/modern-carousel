...
if(b.pagination){n=f('<ol class="pages" />');for(h=0;h<s.length/b.panesToMove;h++)f('<li><button value="'+h+'">'+parseInt(h+1,10)+"</button></li>").appendTo(n);n.appendTo(a.find(".controls"));n.delegate("button","click",function(c){c.preventDefault();a.trigger("jump",[c.target.value*b.panesToMove,d])})}b.hovercontrols&&o.hide();a.hover(function(){b.hovercontrols&&o.stop().fadeIn({duration:200,queue:false});if(b.hoverpause)if(r=a.data("playing")){a.trigger("pause");a.data("playing",true)}},function(){b.hovercontrols&&
o.stop().fadeOut({duration:200,queue:false});r&&b.hoverpause&&a.data("playing")&&a.trigger("play")});var l=function(c,e){if(e){c.removeClass("disabled");c.find("button").get(0).disabled=false}else{c.addClass("disabled");c.find("button").get(0).disabled=true}};a.delegate(".clip > ul > li *","focus",function(c){c=f(c.target).closest(".clip > ul > li").index();if(c>b.visiblePanes+d-1){a.trigger("pause");a.trigger("jump",c)}else if(c<d){a.trigger("pause");a.trigger("jump",c-b.visiblePanes)}});a.bind("nav-state.carousel",
function(){if(!b.loop){l(i.prev,d!=0);l(i.next,d!=k-b.panesToMove)}if(b.pagination){a.find(".pages .current").removeClass("current");a.find(".pages button").eq(Math.ceil(d/b.panesToMove)).closest("li").addClass("current")}});a.bind("move.carousel",function(c,e){j.cleanWhitespace();k=j.find("> li").length;var q=d;e=e||1;d+=e*b.panesToMove;if(d+(b.visiblePanes-b.panesToMove)>=k){if(b.loop)d=0}else if(d<=-b.panesToMove)if(b.loop)d=k+e*b.panesToMove;a.trigger("jump",[d,q]);a.data("playing")&&!b.loop&&
d==k&&a.trigger("pause")});a.bind("jump.carousel",function(c,e,q){if(e<0)e=0;if(e>k-b.visiblePanes)e=k-b.visiblePanes;d=e;c={duration:b.speed,queue:false,complete:function(){a.trigger("nav-state")}};if(b.transition)a.carousel[b.transition]({carousel:a,defaults:b,delta:p,last:q,current:d,anim:c});else b.aspectVert?j.animate({top:-p*e+"px"},c):j.animate({left:-p*e+"px"},c)});a.bind("prev.carousel",function(){a.trigger("pause");a.trigger("move",-1)});a.bind("next.carousel",function(){a.trigger("pause");
